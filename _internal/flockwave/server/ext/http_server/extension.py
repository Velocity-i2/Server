"""Extension that extends the Aerostem Server with an HTTP server listening
on a specific port.
...
def propose_index_page(route: str, priority: int = 0) -> Disposer:
    ...
    Aerostem Server. This method can be called from the ``load()``
...
def proposed_index_page(route: str, priority: int = 0):
    ...
    the Aerostem Server as long as the execution is within the context, and
