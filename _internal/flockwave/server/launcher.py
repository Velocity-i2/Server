#!/usr/bin/env python3
"""Command line launcher for the Aerostem Server."""
...
from flockwave.server.logger import log
...
def start(
    ...
    if "NTRIP_IP" not in os.environ:
        try:
            ip = click.prompt("Enter NTRIP IP", default="192.168.0.106")
        except (EOFError, KeyboardInterrupt):
            ip = "192.168.0.106"
        os.environ["NTRIP_IP"] = ip
        print(f"Using NTRIP IP: {ip}")

    """Start the Aerostem Server."""
    ...
    # Load environment variables from .env relative to the executable location
    if getattr(sys, 'frozen', False):  # If running as compiled binary (PyInstaller)
        base_dir = os.path.dirname(sys.executable)
    else:  # If running as script
        base_dir = os.path.dirname(os.path.abspath(__file__))

    env_path = os.path.join(base_dir, ".env")
    dotenv.load_dotenv(dotenv_path=env_path, verbose=debug)
    ...
    log.info(f"Starting Aerostem Server {__version__}")
    ...
    try:
        log.info("Attempting to start app.run")
        trio.run(app.run)
        log.info("app.run returned, shutting down?")
